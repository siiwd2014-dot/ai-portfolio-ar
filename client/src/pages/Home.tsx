...
const projects = [
  {
    id: "01",
    category: "صورة مولّدة",
    filter: "image",
    type: "IMAGE",
    title: "كوكب الذاكرة",
    subtitle: "هوية بصرية لمنتج تقني يتخيّل الذاكرة كمساحة قابلة للتشكيل.",
    image: "/manus-storage/nova-portrait_18db5980.jpg",
    accent: "amber",
    stat: "12K+",
    statLabel: "توليفة بصرية",
  },
  {
    id: "02",
    category: "فيديو تجريبي",
    filter: "video",
    type: "MOTION",
    title: "ما بعد الضوء",
    subtitle: "فيلم قصير عن مدينة تعيد كتابة شكلها كل 3 ثوانٍ.",
    image:
      "https://images.unsplash.com/photo-1518709268805-4e9042af9f23?auto=format&fit=crop&w=1200&q=85",
    accent: "lime",
    stat: "00:43",
    statLabel: "مدة التجربة",
  },
  {
    id: "03",
    category: "تجربة ويب",
  ...
  },
  {
    ...
    type: "IMAGE",
    title: "صدى اصطناعي",
    subtitle: "دراسة بصرية عن العلاقة بين الوجه البشري والنماذج التنبؤية.",
    image: "/manus-storage/nova-portrait_18db5980.jpg",
    accent: "blue",
    stat: "36",
    statLabel: "نسخة مخرجة",
  },
  {
    id: "05",
    category: "فيديو تجريبي",
  ...
  },
  {
    ...
    subtitle: "منصة صغيرة تساعد الفرق على تحويل الأفكار إلى مسارات واضحة.",
    image:
      "https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1200&q=85",
    accent: "cyan",
    stat: "09",
    statLabel: "تجارب مدمجة",
  },
];
        ...
        <div className="fixed inset-0 z-[70] grid place-items-center bg-black/75 p-4 backdrop-blur-md" role="dialog" aria-modal="true" onClick={() => setSelectedProject(null)}>
          <div className="relative max-h-[90vh] w-full max-w-3xl overflow-auto rounded-[2rem] border border-white/15 bg-[#101a1c] p-3 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="grid gap-8 p-5 md:grid-cols-[1fr_auto] md:items-end">
              ...
              {selectedProject.filter === "web" ? <a href={selectedProject.link} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#d8ff61] px-5 py-3 text-xs font-bold text-[#091113]">فتح صفحة التجربة <ExternalLink className="h-4 w-4" /></a> : <a href="#contact" onClick={() => setSelectedProject(null)} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#d8ff61] px-5 py-3 text-xs font-bold text-[#091113]">اطلب تجربة مشابهة <ExternalLink className="h-4 w-4" /></a>}
            </div>
          </div>
        </div>
      )}
